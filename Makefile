...
$(SERVER):		$(OBJS_SERVER) $(LIBFT)
			@/bin/echo -n "$(shell tput setaf 225)Compiling \"$(SERVER)\": $(shell tput sgr0)"
...
norminette:
			@if norminette $(SRCS_CLIENT) $(SRCS_SERVER) $(INCS) ./minilibft > /dev/null; then \
				echo "$(shell tput setaf 225)Norminette: $(shell tput setaf 222)[OK]$(shell tput sgr0)"; \
			else \
				echo "$(shell tput setaf 225)Norminette: $(shell tput setaf 210)[KO]$(shell tput sgr0)"; \
			fi
			@sleep 0.2

clean:
			@/bin/echo -n "$(shell tput setaf 225)Deleting project's object files: $(shell tput sgr0)"
				@${RM} $(SRCS_CLIENT:.c=.o) $(SRCS_SERVER:.c=.o)
			@echo "$(shell tput setaf 222)[OK]$(shell tput sgr0)"
			@/bin/echo -n "$(shell tput setaf 225)Deleting Minilibft's object files: $(shell tput sgr0)"
				@make clean -sC minilibft > /dev/null
			@echo "$(shell tput setaf 222)[OK]$(shell tput sgr0)"

fclean:			clean
			@/bin/echo -n "$(shell tput setaf 225)Deleting project's binary files: $(shell tput sgr0)"
				@${RM} $(CLIENT) $(SERVER)
			@echo "$(shell tput bold)$(shell tput setaf 222)[OK]$(shell tput sgr0)"
			@/bin/echo -n "$(shell tput setaf 225)Deleting minilibft.a: $(shell tput sgr0)"
				@${RM} $(LIBFT)
			@echo "$(shell tput setaf 222)[OK]$(shell tput sgr0)"
